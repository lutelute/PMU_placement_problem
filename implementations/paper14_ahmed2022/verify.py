"""
Verification script for Paper 14: Ahmed et al. (2022)

"A Critical Review of State-of-the-Art Optimal PMU Placement Techniques"
Energies, Vol. 15, 2125, 2022.

This is a REVIEW paper. It does not present novel PMU placement results
with specific numerical tables of PMU counts. The paper provides:
- Table 1: SCADA vs PMU comparison
- Tables 2-4: Summaries of LP-based, GA-based, and PSO-based OPP methods
  (listing references, techniques, constraints, but NOT numerical results)

The paper states that IEEE 14-bus, 30-bus, 57-bus, and 118-bus are the
most commonly used testbed systems (Section 5.1).

Since no specific PMU count tables are provided, we verify the standard
BILP optimal PMU counts that serve as the consensus reference across the
many papers surveyed in this review.

Consensus results from the literature surveyed:
  Without ZI:                    With ZI:
...
def run_verification():
    """Verify consensus results referenced by the Ahmed 2022 review."""
    print("=" * 60)
    print("Paper 14: Ahmed et al. (2022) -- Critical Review")
    print("=" * 60)
    print()
    print("This is a review paper. Verifying the consensus PMU placement")
    print("results that the reviewed literature converges on.\n")

    # ------------------------------------------------------------------
    # Consensus: WITHOUT Zero Injections
    # ------------------------------------------------------------------
    v1 = PaperVerification(
        paper_id="paper14_ahmed2022_consensus_no_zi",
        paper_title="Ahmed 2022 Review -- Consensus (no ZI)",
        paper_year=2022,
    )
    ...
    v2 = PaperVerification(
        paper_id="paper14_ahmed2022_consensus_with_zi",
        paper_title="Ahmed 2022 Review -- Consensus (with ZI)",
        paper_year=2022,
    )
    ...
    print(f"\n{'=' * 60}")
    print(f"Paper 14 Overall: {n_pass}/{n_total} checks passed")
    print(f"{'=' * 60}")
